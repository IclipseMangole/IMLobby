            ...
            if (friends.size() > 0) {
                for (int i = 0; i < shown.size(); i++) {
                    ...
                    if (User.isOnline(friend)) {
                        menu.addMenuItem(FriendItems.onlineHead(p, friend, page), i);
                    } else {
                        menu.addMenuItem(FriendItems.offlineHead(p, friend, page), i);
                    }
                }
            } else {
                menu.addMenuItem(FriendItems.noFriendsItem(p), 4, 2);
            }
            menu.addMenuItem(FriendItems.requestItem(p, Friend.getRequests(UUIDFetcher.getUUID(p.getName())).size(), page), 5, 5);
            menu.addMenuItem(FriendItems.sortItem(p, page), 6, 5);
