...
public class FriendSortMenu {
    public static void openFriendSortMenu(Player p, int page) {
        PopupMenu menu = ProfileMenu.createSubProfileMenu(dsp.get("friend_menu.sort.title", p), p, 2, new BackAction() {
            @Override
            public void onBack(Player player) {
                FriendMenu.openFriendMenu(player, page);
            }
        });
        ...
        for (int i = 0; i < 9; i++) {
            menu.addMenuItem(FriendItems.toggleItem(p, i, page), i, 3);
        }
        menu.openMenu(p);
    }
}
